...
import { Plus, Trash2, Search, Filter, ShoppingBag, BookOpen, Coffee, HelpCircle, Sparkles, Smile, Plane, Utensils, Tag, Globe, Compass, TrendingUp, TrendingDown } from 'lucide-react-native';
import { Expense, BankAccount, BudgetCategory, Currency } from '../types';
...
interface ExpensesTrackerProps {
  ...
  quickFillCurrency: Currency;
  onClearQuickFill: () => void;
  exchangeRate: number;
}
...
export default function ExpensesTracker({ expenses, accounts, categories, onAddExpense, onDeleteExpense, onAddCategory, quickFillAmount, quickFillCurrency, onClearQuickFill, exchangeRate }: ExpensesTrackerProps) {
  ...
  React.useEffect(() => {
    ...
    if (profileAccounts.length > 0) {
      ...
      if (!currentInProfile) { setAccountId(profileAccounts[0].id); setCurrency(profileAccounts[0].currency); }
    }
  ...
  React.useEffect(() => {
    if (quickFillAmount > 0) {
      setAmount(quickFillAmount.toString());
      setCurrency(quickFillCurrency);
      setActiveProfile(quickFillCurrency);
      const matchedAccount = accounts.find(a => a.currency === quickFillCurrency);
      if (matchedAccount) setAccountId(matchedAccount.id);
      onClearQuickFill();
    }
  }, [quickFillAmount, quickFillCurrency]);
  ...
  const handleCreateCategory = () => {
    ...
    const newCategory: BudgetCategory = { id: `cat-${Date.now()}`, name: newCatName.trim(), icon: newCatIcon, color: newCatColor, isCustom: true };
  ...
  const handleAddExpenseSubmit = () => {
    ...
    onAddExpense({ date, amount: parsedAmount, currency, category: selectedCategory, accountId, note: note.trim() || `${selectedCategory} ${transactionType === 'income' ? 'income' : 'spending'}`, type: transactionType });
    setAmount('');
    setNote('');
  };
  ...
  const filteredExpenses = expenses.filter(exp => {
    ...
    const matchesSearch = exp.note.toLowerCase().includes(searchQuery.toLowerCase()) || exp.category.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesCategory = filterCategory === 'all' || exp.category === filterCategory;
    const matchesAccount = filterAccount === 'all' || exp.accountId === filterAccount;
    return matchesSearch && matchesCategory && matchesAccount;
  });

  const categorySpendingSums = expenses.filter(exp => exp.currency === activeProfile).reduce((acc, exp) => {
    acc[exp.category] = (acc[exp.category] || 0) + exp.amount;
    return acc;
  }, {} as Record<string, number>);

  const totalSpendingInProfileCurrency = Object.values(categorySpendingSums).reduce((sum, val) => sum + val, 0);

  const symbol = activeProfile === 'KRW' ? '₩' : 'RM';
    ...
    <View className="gap-6 pb-4">
      ...
      <View className="bg-stone-50 p-4 rounded-3xl border border-stone-200">
        ...
        <View className="flex-row bg-stone-200/50 p-1 rounded-2xl border border-stone-200 mt-3">
          <TouchableOpacity onPress={() => setActiveProfile('KRW')} className={`flex-1 flex-row items-center justify-center gap-1.5 px-4 py-2 rounded-xl ${activeProfile === 'KRW' ? 'bg-white shadow-sm' : ''}`}>
            <Globe size={14} color="#2563eb" />
            <Text className={`text-xs font-bold ${activeProfile === 'KRW' ? 'text-stone-900' : 'text-stone-500'}`}>KR Accounts</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActiveProfile('MYR')} className={`flex-1 flex-row items-center justify-center gap-1.5 px-4 py-2 rounded-xl ${activeProfile === 'MYR' ? 'bg-white shadow-sm' : ''}`}>
      ...
      <View className="bg-white rounded-3xl p-6 shadow-sm border border-stone-200">
        ...
        <View className="mb-4">
          ...
          <View className="flex-row gap-1 bg-stone-100 p-1 rounded-xl">
            <TouchableOpacity onPress={() => setTransactionType('expense')} className={`flex-1 py-1.5 rounded-lg flex-row justify-center items-center gap-1.5 ${transactionType === 'expense' ? 'bg-red-500' : ''}`}>
            ...
            <TouchableOpacity onPress={() => setTransactionType('income')} className={`flex-1 py-1.5 rounded-lg flex-row justify-center items-center gap-1.5 ${transactionType === 'income' ? 'bg-green-600' : ''}`}>
        ...
        <View className="flex-row gap-3 mb-4">
          <View className="flex-1">
            <Text className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-1">{transactionType === 'income' ? 'Deposit To' : 'Paid From'}</Text>
            <View className="border border-stone-200 rounded-xl overflow-hidden bg-stone-50">
              <Picker selectedValue={accountId} onValueChange={(v) => handleAccountChange(v)} style={{ height: 44 }}>
        ...
        <View className="mb-4">
          <View className="flex-row justify-between items-center mb-1">
            ...
            <TouchableOpacity onPress={() => setShowCategoryCreator(!showCategoryCreator)} className="flex-row items-center gap-0.5">
          ...
          {!showCategoryCreator ? (
            <View className="border border-stone-200 rounded-xl overflow-hidden bg-stone-50">
              <Picker selectedValue={selectedCategory} onValueChange={(v) => setSelectedCategory(v)} style={{ height: 44 }}>
              ...
              <View>
                ...
                <View className="flex-row flex-wrap gap-1">
                  {PRESET_ICONS.map(pi => {
                    ...
                    return (
                      <TouchableOpacity key={pi.name} onPress={() => setNewCatIcon(pi.name)} className={`p-1.5 rounded-md border ${isSelected ? 'bg-stone-800 border-stone-800' : 'bg-white border-stone-200'}`}>
              ...
              <View>
                ...
                <View className="flex-row gap-2">
                  {PRESET_COLORS.map(pc => {
                    ...
                    return (
                      <TouchableOpacity key={pc.name} onPress={() => setNewCatColor(pc.name)} style={{ width: 18, height: 18, borderRadius: 9, backgroundColor: pc.iconColor, borderWidth: isSelected ? 2.5 : 0, borderColor: '#1c1917' }} />
                    );
                  })}
                </View>
              </View>
        ...
        <TouchableOpacity onPress={handleAddExpenseSubmit} className="w-full bg-green-500 py-2.5 px-4 rounded-xl items-center">
      ...
      <View className="bg-white rounded-3xl p-6 shadow-sm border border-stone-200">
        ...
        <View className="gap-3">
          {categories.map(c => {
            const spentInProfile = categorySpendingSums[c.name] || 0;
            const percentage = totalSpendingInProfileCurrency > 0 ? (spentInProfile / totalSpendingInProfileCurrency) * 100 : 0;
              ...
              <View key={c.id} className="gap-1">
                <View className="flex-row justify-between items-center">
                  ...
                  <Text className="text-stone-500 text-xs font-medium">
                    {percentage.toFixed(0)}% ({symbol} {activeProfile === 'KRW' ? Math.round(spentInProfile).toLocaleString() : spentInProfile.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                  </Text>
                </View>
                <View className="w-full bg-stone-100 rounded-full h-1.5 overflow-hidden">
                  <View className={`h-full ${colorConfig.bg} rounded-full`} style={{ width: `${percentage}%` }} />
                </View>
              </View>
            );
          })}
          {expenses.filter(exp => exp.currency === activeProfile).length === 0 && (
            <Text className="text-xs text-stone-400 text-center py-4">No {activeProfile === 'KRW' ? 'Korean' : 'Malaysian'} expenses recorded yet.</Text>
          )}
        </View>
      </View>
      ...
      <View className="bg-white rounded-3xl p-6 shadow-sm border border-stone-200">
        ...
        <View className="flex-row items-center bg-stone-50 border border-stone-200 rounded-xl px-3 py-1.5 gap-2 mb-3">
          ...
          <TextInput className="flex-1 text-xs" placeholder="Search note or label..." value={searchQuery} onChangeText={setSearchQuery} />
        </View>
        ...
        <View className="flex-row gap-2 mb-4 flex-wrap">
          ...
          <View className="border border-stone-200 rounded-lg bg-stone-50 overflow-hidden">
            <Picker selectedValue={filterAccount} onValueChange={(v) => setFilterAccount(v)} style={{ height: 32, width: 140 }}>
              <Picker.Item label="All Accounts" value="all" />
              {accounts.filter(a => a.currency === activeProfile).map(a => <Picker.Item key={a.id} label={a.bankName} value={a.id} />)}
            </Picker>
          </View>
          <View className="border border-stone-200 rounded-lg bg-stone-50 overflow-hidden">
            <Picker selectedValue={filterCategory} onValueChange={(v) => setFilterCategory(v)} style={{ height: 32, width: 150 }}>
                ...
                <View key={exp.id} className="flex-row items-center justify-between p-3.5 bg-stone-50 rounded-2xl border border-stone-200">
                  ...
                  <View className="flex-row items-center gap-2">
                    ...
                    <Text className={`font-bold text-xs ${isPositive ? 'text-green-600' : 'text-red-500'}`}>
                      {isPositive ? '+' : '-'} {exp.currency === 'KRW' ? '₩' : 'RM'} {Math.abs(exp.amount).toLocaleString(undefined, { minimumFractionDigits: exp.currency === 'KRW' ? 0 : 2 })}
                    </Text>
                    <TouchableOpacity onPress={() => onDeleteExpense(exp.id)} className="p-1">
        ...
        <View className="mt-4 pt-4 border-t border-stone-200 items-end">
          ...
          <Text className="font-bold text-stone-900 text-xl">
            {symbol} {activeProfile === 'KRW' ? Math.round(totalSpendingInProfileCurrency).toLocaleString() : totalSpendingInProfileCurrency.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Text>
          <Text className="text-xs text-stone-400 mt-0.5 font-medium">
            ≈ {activeProfile === 'KRW' ? 'RM' : '₩'} {activeProfile === 'KRW'
              ? ((totalSpendingInProfileCurrency / 1000) * exchangeRate).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
              : Math.round(totalSpendingInProfileCurrency * (1000 / exchangeRate)).toLocaleString()}
          </Text>
        </View>
      </View>
    </View>
