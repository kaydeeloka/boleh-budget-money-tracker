...
import { Expense, BankAccount, BudgetCategory } from '../types';

interface StatsAndCalendarProps {
  expenses: Expense[];
...
export default function StatsAndCalendar({ expenses, accounts, categories }: StatsAndCalendarProps) {
  const now = new Date();
  ...
  const getExpensesForDate = (day: number) => {
    ...
    return filteredExpenses.filter(e => e.date === dateStr);
  };

  const getDayTotal = (day: number) => {
    const dayExps = getExpensesForDate(day);
  ...
  const categoryTotals = categories.map(cat => {
    const total = monthExpenses.filter(e => e.category === cat.name && e.type !== 'income').reduce((s, e) => s + e.amount, 0);
    return { ...cat, total };
  ...
  const weeklyTotals = [3, 2, 1, 0].map(weeksAgo => {
    ...
    const label = `W${Math.ceil((end.getDate()) / 7)}`;
    return { label, total };
  });
    ...
    <View className="gap-6 pb-4">
      ...
      <View className="bg-stone-50 p-4 rounded-3xl border border-stone-200">
        <View className="flex-row items-center gap-2 mb-3">
          ...
          <Text className="font-bold text-stone-800 text-lg">Spending Stats & Calendar</Text>
        </View>
        <View className="flex-row bg-stone-200/50 p-1 rounded-2xl border border-stone-200">
          <TouchableOpacity onPress={() => setActiveCurrency('KRW')} className={`flex-1 py-1.5 rounded-xl items-center ${activeCurrency === 'KRW' ? 'bg-white shadow-sm' : ''}`}>
            <Text className={`text-xs font-bold ${activeCurrency === 'KRW' ? 'text-stone-900' : 'text-stone-500'}`}>₩ KRW (Korea)</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActiveCurrency('MYR')} className={`flex-1 py-1.5 rounded-xl items-center ${activeCurrency === 'MYR' ? 'bg-white shadow-sm' : ''}`}>
      ...
      <View className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm">
        <Text className="text-[10px] font-bold text-stone-400 uppercase mb-1">Net ({MONTH_NAMES[viewMonth]} {viewYear})</Text>
      ...
      <View className="bg-white rounded-3xl p-6 shadow-sm border border-stone-200">
        <View className="flex-row items-center gap-2 mb-4">
          ...
          <Text className="text-sm font-bold text-stone-800 w-32 text-center">{MONTH_NAMES[viewMonth].slice(0, 3)} {viewYear}</Text>
            ...
            return (
              <View key={day} style={{ width: '14.285714%', aspectRatio: 1, padding: 2 }}>
                <TouchableOpacity
                  ...
                  style={{ flex: 1, borderRadius: 8, alignItems: 'center', justifyContent: 'center', position: 'relative', backgroundColor: isSelected ? '#22C55E' : isToday ? '#ecfdf5' : '#fafaf9', borderWidth: isToday && !isSelected ? 1 : 0, borderColor: '#6ee7b7' }}
                >
